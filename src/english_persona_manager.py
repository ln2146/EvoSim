#!/usr/bin/env python3
"""
English persona management system
Select suitable personas from the English database for different agent types
Supports 200 positive, 200 neutral, and 200 negative personas
"""
...
class EnglishPersonaManager:
    def __init__(self):
        ...
        logger.info(f"✅ English persona database loaded:")
        logger.info(f"   Positive personas: {len(self.positive_personas)}")
        logger.info(f"   Neutral personas: {len(self.neutral_personas)}")
        logger.info(f"   Negative personas: {len(self.negative_personas)}")
        logger.info(f"   Total: {len(self.positive_personas) + len(self.neutral_personas) + len(self.negative_personas)}")
    ...
    def load_personas(self, filename: str) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"❌ Failed to load personas from {filename}: {e}")
            return []
    ...
    def reset_used_personas(self):
        ...
        logger.info("🔄 Reset persona usage records")
    ...
    def select_persona_for_balance_agent(self) -> Optional[Dict]:
        ...
        if not available_positive:
            logger.warning("⚠️ No available positive personas, resetting usage records")
            self.reset_used_personas()
            available_positive = self.positive_personas
        
        # Prefer positive personas with specific traits
        preferred_traits = ["Rational", "Tolerant", "Gentle", "Upright", "Compassionate"]
        ...
        logger.info(f"🎭 Selected persona for balance agent: {selected['name']} ({selected['demographics']['profession']})")
        return selected
    ...
    def select_persona_for_regular_user(self) -> Optional[Dict]:
        ...
        if not available:
            logger.warning(f"⚠️ No available {source_name} personas, resetting usage records")
            self.reset_used_personas()
            if persona_type == 'positive':
    ...
    def select_persona_for_malicious_agent(self) -> Optional[Dict]:
        ...
        if not available_negative:
            logger.warning("⚠️ No available negative personas, resetting usage records")
            self.reset_used_personas()
            available_negative = self.negative_personas
        
        # Prefer negative personas with specific traits
        preferred_traits = ["Aggressive", "Extreme", "Inflammatory", "Paranoid", "Angry"]
    ...
    def get_persona_statistics(self) -> Dict:
        ...
        # Usage statistics by persona type
    ...
    def print_statistics(self):
        ...
        print("📊 English Persona Usage Statistics:")
        ...
        for persona_type, type_stats in stats['by_type'].items():
            ...
            print(f"   {type_name}: {type_stats['used']}/{type_stats['total']} (remaining {type_stats['available']})")
...
def main():
    """Testing English persona management system"""
    manager = EnglishPersonaManager()
    
    print("🧪 Testing English Persona Selection System...")
    print()
    
    # Test balance agent persona selection
    print("1. Testing balance agent persona selection:")
    balance_persona = manager.select_persona_for_balance_agent()
    if balance_persona:
        print(f"   Selected: {balance_persona['name']}")
        print(f"   Profession: {balance_persona['demographics']['profession']}")
        print(f"   Traits: {', '.join(balance_persona['personality_traits'])}")
    print()
    
    # Test regular user persona selection
    print("2. Testing regular user persona selection (1:1:1 ratio):")
    for i in range(6):
        user_persona = manager.select_persona_for_regular_user()
        if user_persona:
            print(f"   User{i+1}: {user_persona['name']} ({user_persona['type']}) - {user_persona['demographics']['profession']}")
    print()
    
    # Test malicious agent persona selection
    print("3. Testing malicious agent persona selection:")
    malicious_persona = manager.select_persona_for_malicious_agent()
    if malicious_persona:
        print(f"   Selected: {malicious_persona['name']}")
        print(f"   Profession: {malicious_persona['demographics']['profession']}")
        print(f"   Traits: {', '.join(malicious_persona['personality_traits'])}")
    print()
