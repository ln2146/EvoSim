...
import { Layers, Users, AlertTriangle, TrendingUp, Network, GitBranch } from 'lucide-react'
...
export default function FilterBubbleObservation() {
  ...
  const [viewMode, setViewMode] = useState<'overview' | 'user-detail' | 'factions'>('overview')
  const [postFactions, setPostFactions] = useState<PostFactionsSummary | null>(null)
  const [activeTab, setActiveTab] = useState<'bubble' | 'factions'>('bubble')
  ...
  const loadFactionsData = async () => {
    setLoading(true)
    try {
      const data = await getPostFactions(selectedDb, 50, 0)  // 修改为0，显示所有帖子
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex bg-slate-100 rounded-xl p-1">
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  viewMode === 'overview' || viewMode === 'user-detail'
      ...
      {viewMode === 'factions' && postFactions && (
        <>
          {/* 返回按钮 */}
          <div className="glass-card p-4">
            <button
          ...
          {/* 帖子派系分析概览 */}
          <div className="glass-card p-6">
            <h2 className="text-2xl font-bold text-slate-800 mb-6">帖子派系分布分析</h2>
                ...
                <div className="glass-card p-6">
                  ...
                  <div className="space-y-4 max-h-[600px] overflow-y-auto">
                    {(postFactions.post_stances || postFactions.post_factions || []).map((post) => {
                      // 使用正确的总数：优先使用 total_comments，否则使用 total_interactions
                      const totalForCalculation = post.total_comments ?? post.total_interactions;

                      // 获取评论数量（优先使用新字段，否则从比率计算）
