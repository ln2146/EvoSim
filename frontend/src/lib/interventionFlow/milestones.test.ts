...
describe('toUserMilestone', () => {
  ...
  it('maps Amplifier lines', () => {
    ...
    expect(toUserMilestone('⚖️ Activating Amplifier Agent cluster...')).toBe('扩散者：启动集群')
    expect(toUserMilestone('🚀 Start parallel execution of 12 agent tasks...')).toBe('扩散者：并行执行（12）')
    expect(toUserMilestone('📊 amplifier Agent results: 12 succeeded, 0 failed')).toBe('扩散者：执行结果（成功 12 / 失败 0）')
    expect(toUserMilestone('📊 Amplifier Agent results: 12 succeeded, 0 failed')).toBe('扩散者：执行结果（成功 12 / 失败 0）')
    expect(toUserMilestone('📋 amplifier plan: total=12, role distribution={...}')).toBe('扩散者：集群规模（12）')
    expect(toUserMilestone('📋 Amplifier plan: total=12, role distribution={...}')).toBe('扩散者：集群规模（12）')
    expect(toUserMilestone('✅ 12 amplifier responses generated')).toBe('扩散者：生成回应（12）')
    expect(toUserMilestone('✅ 12 amplifier responses generated')).toBe('扩散者：生成回应（12）')
    expect(toUserMilestone('💖 Successfully added 240 likes to each of 2 leader comments (total: 480 likes)')).toBeNull()
    expect(toUserMilestone('💖 12 Amplifier Agents liked leader comments')).toBe('扩散者：点赞扩散（12）')
