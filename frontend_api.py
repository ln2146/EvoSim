...
def detect_factions():
    ...
    try:
        ...
        report = detector.detect_factions(
        ...
        communities_data = [
            {
                ...
                'cross_community_links': c.cross_community_links,
                'members': c.members[:10]  # 只返回前10个成员作为示例
            }
        ...
        return jsonify({
            ...
            'modularity': {
                'modularity': report.modularity.modularity,
                'num_communities': report.modularity.num_communities,
                'avg_community_size': report.modularity.avg_community_size,
                'max_community_size': report.modularity.max_community_size,
                'partition_quality': report.modularity.partition_quality
            },
            'network_stats': report.network_stats,
            'faction_map_sample': dict(list(report.faction_map.items())[:20])  # 样本
        })
    except Exception as e:
        import traceback
        traceback.print_exc()
...
def get_post_factions():
    ...
    try:
        ...
        min_comments = int(request.args.get('min_comments', 3))
        ...
        detector = CommunityDetector(db_path)
        summary = detector.get_post_factions_summary(
            limit=limit,
...
def get_cross_faction_interactions():
    ...
    try:
        ...
        detector = CommunityDetector(db_path)
        interactions = detector.get_cross_faction_interactions()
        ...
        return jsonify(interactions_str)
    except Exception as e:
        import traceback
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500
...
def get_echo_chamber_users():
    ...
    try:
        ...
        detector = CommunityDetector(db_path)
        echo_users = detector.get_echo_chamber_users(threshold=threshold)
